...
datamanager = torchreid.data.ImageDataManager(
        root='reid-data',
        sources="msmt17",
        targets=["cuhk03", "market1501", "dukemtmcreid"],
        height=256,
        width=128,
...
model_student, model_grl_student, optimizer_student, scheduler_student, start_epoch = initialize_model_optimizer_scheduler(
        name='resnet18', num_classes=datamanager.num_train_pids, loss='mmd', pretrained=True,
        optim='adam', lr=0.0003,
        lr_scheduler='single_step', stepsize=50,
        path_model="log/msmt_to_all3/t_resnet34_s_resnet18/source_msmt17.pth.tar-120"
)

# path_target = ["log/market_to_all3/student_18_teacher_34/target_cuhk03.pth.tar-139", "log/market_to_all3/student_18_teacher_34/target_msmt17.pth.tar-139", "log/market_to_all3/student_18_teacher_34/target_dukemtmcreid.pth.tar-139"]
path_target = ["log/source_training/msmt17/resnet34/source_msmt17.pth.tar-90", "log/source_training/msmt17/resnet34/source_msmt17.pth.tar-90", "log/source_training/msmt17/resnet34/source_msmt17.pth.tar-90"]
...
for i in range(0, len(datamanager.targets)):
        model, model_grl, optimizer, scheduler, start_epoch = initialize_model_optimizer_scheduler(
                name='resnet34', num_classes=datamanager.num_train_pids, loss='mmd', pretrained=True,
                optim='adam', lr=0.0003,
                lr_scheduler='single_step', stepsize=50,
                path_model=path_target[i]
        )
