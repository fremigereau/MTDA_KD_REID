...
def initialize_model_optimizer_scheduler(
        ...
        # model = torch.nn.DataParallel(model).to(device=torch.device('cuda:0'))
        ...
        if path_model != None:
                ...
                model = model.cuda()
                # model = torch.nn.DataParallel(model).cuda()  # Comment previous line and uncomment this line for multi-gpu use
                # model = torch.nn.DataParallel(model).to(device=torch.device('cuda:0'))
...
model_student, model_grl_student, optimizer_student, scheduler_student, start_epoch = initialize_model_optimizer_scheduler(
        ...
        path_model='log/source_training/msmt17/resnet18/source_msmt17.pth.tar-60'

)
# path_target = ["log/source_training/msmt17/resnet34/source_msmt17.pth.tar-40", "log/source_training/market1501/resnet34/source_market1501.pth.tar-40", "log/source_training/market1501/resnet34/source_market1501.pth.tar-40"]
...
engine.run(
        save_dir='log/msmt_to_all3/t_resnet18_s_resnet18',
        max_epoch=200,
        eval_freq=10,
        print_freq=10,
        test_only=False,
